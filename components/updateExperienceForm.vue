<template>
    <div class="w-[92%] border-t border-[#23282d] py-2 space-y-4 lg:space-y-6">
      ...
      <div class="w-full flex flex-row items-center justify-between space-x-2 lg:space-x-4">
        ...
        <div class="w-1/2 flex flex-col items-start justify-center ">
          ...
          <BaseSelect :options="['1999','2000','2001','2002','2003','2004','2005']" :placeholder="props.experience.yearStarted" @optionChanged="(value:string)=>{ experience.yearStarted = value}" />
        </div>
        <!-- TO -->
        <div class="w-1/2 flex flex-col items-start justify-center ">
          <h2 class="text-sm text-[#555a5c] text-left ml-3 mb-2">To<span class="text-red-600 text-lg">*</span></h2>
          <BaseSelect :options="['1999','2000','2001','2002','2003','2004','2005','Now']" :placeholder="props.experience.yearEnded" @optionChanged="(value:string)=>{ experience.yearEnded = value}" />
        </div>
  ...
  const emit = defineEmits(['updateExperience','cancel'])  // Declare Events
  ...
  const experience = reactive(
    {
      yearStarted:props.experience.yearStarted || '',
      yearEnded:props.experience.yearEnded || '',
      title:props.experience.title || '',
      company: props.experience.company || '',
      location: props.experience.location || '',
      url: props.experience.url || '',
      descriptions:[...props.experience.descriptions]
    }
  )

    const submit = ()=>{   //submits and validates form
    if(isFormValid(experience)){
      emit('updateExperience',experience)
