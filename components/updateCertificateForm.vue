<template>
    <div class="w-[92%] border-t border-[#23282d] py-2 space-y-4 lg:space-y-6">
      ...
      <div class="w-full flex flex-row items-center justify-between space-x-2 lg:space-x-4">
        ...
        <div class="w-1/2 flex flex-col items-start justify-center ">
          ...
          <BaseSelect :options="['1999','2000','2001','2002','2003','2004','2005']" :placeholder="props.certificate.yearIssued" @optionChanged="(value:string)=>{ certification.yearIssued = value}" />
        </div>
        <!-- Year Expires -->
        <div class="w-1/2 flex flex-col items-start justify-center ">
          <h2 class="text-sm text-[#555a5c] text-left ml-3 mb-2">Expires<span class="text-red-600 text-lg">*</span></h2>
          <BaseSelect :options="['1999','2000','2001','2002','2003','2004','2005','Never']" :placeholder="props.certificate.yearExpires" @optionChanged="(value:string)=>{ certification.yearExpires = value}" />
        </div>
  ...
  const emit = defineEmits(['updateCertificate','cancel'])  // Declare Events
  ...
  const certification = reactive(
    {
      yearIssued:props.certificate.yearIssued||'',
      yearExpires:props.certificate.yearExpires||'',
      name:props.certificate.name||'',
      organisation:props.certificate.organisation||'',
    ...
    if(isFormValid(certification)){
      emit('updateCertificate',certification)
