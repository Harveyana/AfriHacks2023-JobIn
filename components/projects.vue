<template>
  <div class="w-full">
    <div class="w-full ">
      ...
      <section v-if="display=='project'" class="w-full bg-[#12171d] rounded-3xl flex flex-col items-center justify-center space-y-4 lg:space-y-8 py-6">
        ...
        <div v-if="projects.length" class="w-full h-full bg-[#12171d] rounded-3xl flex flex-col items-center justify-start">
          ...
          <div class="w-full flex flex-row items-center justify-between px-5 mb-4">
           ...
           <Project :projects="projects" @delete="(index)=>{deleteProject(index)}" @update="(index)=>{initProject(index)}" />
      ...
      <section v-if="display=='update'" class="bg-[#0b1015] rounded-3xl w-full flex flex-col items-center justify-center space-y-2 lg:space-y-2 py-4">
        ...
        <!-- Form-->
        <updateProjectForm :project="projectToUpdate" @updateProject="(project)=>{updateProject(project)}"  @cancel="display = 'project'"/> 
    ...
    <div v-if="display=='project'" class="w-full flex flex-row items-center justify-center space-x-8 my-4">
      ...
      <button v-if="!showLoader" @click="$emit('next')" class="extraboldCabinet text-xs lg:text-lg w-1/2 hover:bg-white bg-gray-500 flex flex-row items-center justify-center rounded-3xl p-2.5">
        <span v-if="projects.length" class="text-black">Next</span>
  ...
  const initProject = (index:number)=>{
    projectToUpdate.value = projects.value[index];
  ...
  const updateProject = (data: project): void => {
    // Replace experience at a specific index in the array of experiences
    projects.value[indexToUpdate.value] = data;
    display.value = 'project';
    submit();
