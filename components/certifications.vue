<template>
  <div class="w-full">
    <div class="w-full ">
      ...
      <section v-if="display=='certifications'" class="w-full bg-[#12171d] rounded-3xl flex flex-col items-center justify-center space-y-4 lg:space-y-8 py-6">
        
        <!-- Data -->
        <div v-if="certifications.length" class="w-full h-full bg-[#12171d] rounded-3xl flex flex-col items-center justify-start">
          ...
          <div class="w-full flex flex-row items-center justify-between px-5 mb-4">
           ...
           <Certificates :certifications="certifications" @delete="(index)=>{deleteCertificate(index)}" @update="(index)=>{initCertificate(index)}" />
         
        </div>

        <!-- Alternate Display -->
        <PlaceHolder :step="3" v-else @alternate="display='form'"/>
      ...
      <section v-if="display=='form'" class="bg-[#0b1015] rounded-3xl w-full flex flex-col items-center justify-center space-y-2 lg:space-y-2 py-4">
        ...
        <!-- Form-->
        <certificateForm @addCertificate="(certificate)=>{addCertificate(certificate)}"  @cancel="display = 'certifications'"/> 
      ...
      <section v-if="display=='update'" class="bg-[#0b1015] rounded-3xl w-full flex flex-col items-center justify-center space-y-2 lg:space-y-2 py-4">
        ...
        <!-- Form-->
        <updateCertificateForm :certificate="certificateToUpdate" @updateCertificate="(certificate)=>{updateCertificate(certificate)}"  @cancel="display = 'certifications'"/> 
    ...
    <!-- NavButtons -->
    <div v-if="display=='certifications'" class="w-full flex flex-row items-center justify-center space-x-8 my-4">
      ...
      <button v-if="!showLoader" @click="$emit('next')" class="extraboldCabinet text-xs lg:text-lg w-1/2 hover:bg-white bg-gray-500 flex flex-row items-center justify-center rounded-3xl p-2.5">
        <span v-if="certifications.length" class="text-black">Next</span>
  ...
  const certificateToUpdate = ref<certificate>({
      yearIssued:'',
      yearExpires:'',
      name:'',
      organisation:'',
  ...
  const initCertificate = (index:number)=>{
    certificateToUpdate.value = certifications.value[index];
  ...
  const submit = async()=>{
    ...
    if(certifications.value[0]){
      showLoader.value = true
      const formSaved = await updateUserDetails('certifications',certifications.value)
  ...
  const addCertificate = (data:certificate)=>{   // Add certificate to array of certifications 
    ...
    display.value = 'certifications';
    submit();
    // console.log(certifications)
  }

  const updateCertificate = (data: certificate): void => {
    // Replace experience at a specific index in the array of experiences
    certifications.value[indexToUpdate.value] = data;
    display.value = 'certifications';
    submit();
    // console.log(certifications.value);
  }

  const deleteCertificate = (index:number) => {   // Remove certificate from array of certifications
