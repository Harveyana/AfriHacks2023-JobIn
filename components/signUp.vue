<template>
      <div class="w-full h-fit sm:w-[60%] lg:w-[75%] space-y-8 my-8 sm:space-y-4 overflow-y-scroll no-scrollbar">
        ...
        <div class="w-full flex flex-col items-start justify-start space-y-5 ">
          ...
          <p v-if="showLoader == false" class="text-sm text-[#555a5c] text-center flex flex-row items-center justify-center">
        ...
        <div class="w-full lg:w-[80%] flex flex-col items-start justify-start space-y-4">
          ...
          <form class="w-full flex flex-col items-start justify-start space-y-4 ">
            ...
            <div class="w-full flex flex-col items-start justify-start space-y-1">
              ...
              <div class="w-full flex items-center justify-center border-[#555a5c] border bg-[#12171d] rounded-3xl">
                <input
 ...
 const isValidForm = computed(() =>{ return inputs.name !== '' && isValidEmail(inputs.email) && inputs.password.length >= 6;})

 async function onSuccess(name:string,email:string,password:string) {       // if form is valid
      // if(values.email && values.password && values.name){
        ...
        .then(async (userCredential) => {
          ...
          const profile:user = {uid:user.uid,displayName:name||'',email:email||'',phoneNumber:'',photoUrl:'https://firebasestorage.googleapis.com/v0/b/jobroute-58c44.appspot.com/o/Group%2014%20Copy%202.png?alt=media&token=4233232f-eca6-442d-a5d4-9fd9d0ea5e43'}
  ...
  const submitForm =()=> {
      if(inputs.name = ''){
        ...
        initToast(false,'Full Name required','Please enter Full name')
        return;
      }
      ...
      if (inputs.password.length < 6) {
        ...
        initToast(false,'Password required','Password must be at least 6 characters long')
        return;
      }

      onSuccess(inputs.name,inputs.email,inputs.password)
