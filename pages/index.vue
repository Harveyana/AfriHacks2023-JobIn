<template>
  <div class="w-full h-full z-10 no-scrollbar overflow-y-scroll scroll-smooth bg-[#f6f6f6] dark:bg-[#0b1015]">
    <!-- <Toolbar /> -->
    <!-- Container -->
    <section class="w-full h-[85%] 2xl:h-[70%] my-[16%] sm:my-0 lg:mb-0 grid grid-cols-12 gap-y-2 gap-x-4 pt-2 px-2 sm:px-4 pb-4">
        ...
        <div v-if="!showPreview" class="relative h-full lg:h-[90vh] 2xl:h-[70vh] mb-[2rem] sm:mb-0 flex flex-col items-center justify-start overflow-hidden col-span-12 lg:col-span-9">
          ...
          <div v-if="state.chatResponse.value !==''" class="h-[85%] lg:h-fit lg:min-h-[80%] lg:max-h-[80%]  w-full overflow-hidden flex flex-col space-y-4 bg-white dark:bg-[#12171d] rounded-2xl px-2 py-3">
            ...
            <ChatRequest :expanse="requestExpanse" @seeMore="onExpand()" @seeLess="onshrink()" @openDownload="initializeDownload()" />

            <ChatResponse :expanse="responseExpanse" @seeMore="onshrink()" @seeLess="onExpand()" @openDownload="initializeDownload()" />
          ...
          <div id="Enter description" class="h-fit w-full flex flex-row items-start justify-between gap-x-3 pt-2 pb-4 px-2 rounded-2xl ">
            ...
            <button @click="state.showActions.value = true"  v-if="state.chatRequest.value !==''" class="flex-none border border-black w-12 h-full p-1 sm:px-4 mx-1 hover:bg-gray-600 dark:bg-white hover-bg-gray-200 dark:hover:bg-gray-400 flex items-center justify-center rounded-[50%]">
...
const initializeDownload = async()=>{
  ...
  state.parsedData.value = response
